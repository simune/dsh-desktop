...
| 目标平台 | **macOS(首发)→ Windows x64(2026-08 适配完成)** | spawn/清理/打包均按平台分支收敛;见 docs/05 §4.5 |
| 客户端壳范围 | **壳 + 设置窗口 + 托盘/开机自启** | v1 含原生壳 UI(设置页)、托盘菜单、LoginItems(Windows 为注册表 Run 键) |
| 分发方式 | **个人自用,本地打包** | macOS: dmg;Windows: NSIS(SimpChinese);不做签名公证 |
