...
| 症状 | 排查 | 处理 |
|---|---|---|
| 构建失败 `failed to read file '.../permissions/path/autogenerated/._default.toml': stream did not contain valid UTF-8` | 工作区位于 ExFAT 卷,文件系统为每个文件生成 `._` AppleDouble 侧车,tauri-build 遍历输出目录时读到 | 已内置解决:`dsh-desktop/.cargo/config.toml` 将 cargo target 目录指向 APFS 卷(`~/Library/Caches/dsh-desktop/target`);若迁移到 APFS 卷可删除该文件 |
|---|---|---|
