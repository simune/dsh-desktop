...
| 目标平台 | macOS(首发) + Windows x64(已适配);Linux 仍预留 |
| 客户端壳 | 壳 + 设置窗口 + 托盘 + 开机自启 |
| 分发方式 | 个人自用,本地打包(macOS: dmg;Windows: NSIS),不做签名公证 |
...
| M3.5 | 跨平台:进程组清理/路径/vendor/NSIS;Windows x64 实际适配,Linux 仍预留 | M1.x | 平台分支 + NSIS | Windows 按 docs/05 §4.5 可验收;Linux 仅脚本/cfg 预留 |
